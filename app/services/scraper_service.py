...
def scrape_and_save_recipe(recipe_path: str, recipe_name: str, servings: int) -> int:
    ...
    try:
        ...
        for step in raw_instructions:
            ...
            if step_html:
                clean_step = BeautifulSoup(step_html, "html.parser").get_text(
                    strip=True
                )
        ...
        for item in raw_ingredients_list:
            ...
            if match_pattern_1:
                ...
                if raw_multiplier is not None:
                    ...
                    # Safe parsing for fractions/mixed numbers
                    ...
                    base_quantity = parse_quantity(raw_quantity)
                    if base_quantity is not None:
                        quantity = base_quantity * multiplier

            else:
                # --- Pattern 2: Name xMultiplier (e.g. "White potato x3") ---
                multiplier_match = re.search(
                    r"x(\d+)$", ingredient_label, re.IGNORECASE
                )
        ...
        with current_app.app_context():
            ...
            existing_recipe = db.session.scalar(
                db.select(Recipe).filter_by(name=recipe_name).limit(1)
            )
            ...
            for title in label_titles:
                label_db = db.session.scalar(
                    db.select(Label).filter_by(title=title).limit(1)
                )
                if not label_db:
            ...
            for basic_name in basic_items:
                ingredient_db = db.session.scalar(
                    db.select(Ingredient).filter_by(name=basic_name).limit(1)
                )
            ...
            for item in parsed_ingredients:
                ingredient_db = db.session.scalar(
                    db.select(Ingredient).filter_by(name=item["name"])
                )
        ...
        logging.exception(
            "FATAL ERROR during scraping/database commit for slug '%s'", slug
        )
